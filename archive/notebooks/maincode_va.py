...
'''
function: compute stft's for left, right, and mono
param: x_R (right signal), x_L (left signal)
return: mag_stft, mag_L, mag_R, phase_L, phase_, stft_L, stft_R
'''
def compute_stfts(x_L, x_R):

    stft_left = librosa.stft(x_L, n_fft=n_fft_default, hop_length=HOP_LENGTH)
    stft_right = librosa.stft(x_R, n_fft=n_fft_default, hop_length=HOP_LENGTH)
    ...
    return mag, mag_left, mag_right, phase_left, phase_right, stft_left, stft_right
...
print(f'sample IDs: {sample_idxs}')
...
# parallelize stft jobs
# for each sample: compute and store magnitude spectrum and phase for each channel (as well as mono)
results = joblib.Parallel(n_jobs=4)(joblib.delayed(compute_stfts)(samples_L[i], samples_R[i]) for i in range(n_samples))

# append results
for mag, mag_left, mag_right, phase_left, phase_right, stft_left, stft_right in results:
    mag_mono.append(mag)
...
for i in tqdm.tqdm(range(n_samples)):
    ...
    output_path = f'0{i}_vocal_separated.wav'
