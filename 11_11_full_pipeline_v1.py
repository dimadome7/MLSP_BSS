...
for d in sorted(os.listdir(data_root)):
    track_dir = os.path.join(data_root, d)
...
model = NMF(n_components=n_components, init='nndsvda', random_state=0, max_iter=450, solver='mu', beta_loss='itakura-saito', l1_ratio=0.0)
...
for i in tqdm.tqdm(range(n_samples)):
    ...
    # frequency bins
    freq_bins = librosa.fft_frequencies(sr=sr, n_fft=n_fft_default)

    # vocals in the specified range
    vocal_low, vocal_high = freq_range_hz
    vocal_band_idx = np.where((freq_bins >= vocal_low) & (freq_bins <= vocal_high))[0]

    # time-varying energy in the vocal band from the mixture
    vocal_band_energy_t = np.sum(mag_mono_i[vocal_band_idx, :], axis=0)

    # normalize and zero-mean for correlation
    eps = 1e-9
    vocal_activity = vocal_band_energy_t - np.mean(vocal_band_energy_t)
    vocal_activity /= (np.linalg.norm(vocal_activity) + eps)  # now unit-norm-ish

    # construct scores that represent how 'vocal-like' each component is
    scores = []

    for k in range(n_components):
        
        # compute energy in each channel for panning
        Hk_L = H_L[i][k, :]
        Hk_R = H_R[i][k, :]
        Hk_total = Hk_L + Hk_R
        ...
        # calculate panning score
        pan_score = max(0.0, 1.0 - panning_diff / 0.6)

        # calculate temporal simularity score:
        # insert comment here about Hk_centered
        Hk_centered = Hk_total - np.mean(Hk_total)
        Hk_centered /= (np.linalg.norm(Hk_centered) + eps)

        # calculate simularity to activity in vocal band
        temporal_sim = np.sum(Hk_centered * vocal_activity)

        # keep simularities non negative
        ...
        vocal_band_ratio = energy_vocal_band / total_energy

        # flatness (keep below 0.5)
        flatness = spectral_flattness(basis_spectrum)
        flat_score = max(0.0, (0.5 - flatness))
        ...
        # create congolomerate score
        # weigh scores according to importance
        score = ((pan_weight * pan_score) +     
                 (temp_weight * temp_score) + 
                 (freq_weight * vocal_band_ratio) +
                 (flat_weight * flat_score)
        )

        # hard filter out components without enough vocal energy
        freq_ratio_cutoff = 0.15
        if vocal_band_ratio < freq_ratio_cutoff:
            score = -1e5

        # keep scores for each component
        scores.append(score)

    scores = np.array(scores)

    # number of components we are considering to be vocal
    K = 10

    # use top-k components for reconstruction
    vocal_indices = np.argsort(scores)[-K:]
    instrumental_indices = [idx for idx in range(n_components) if idx not in vocal_indices]

    # then use these in your reconstruction
    W_vocal = W[i][:, vocal_indices]
    H_left_vocal = H_L[i][vocal_indices, :]
    H_right_vocal = H_R[i][vocal_indices, :]

    W_instrumental = W[i][:, instrumental_indices]
    H_left_instrumental = H_L[i][instrumental_indices, :]
    H_right_instrumental = H_R[i][instrumental_indices, :]
    ...
    mag_total_vocal = mag_left_vocal_recon + mag_right_vocal_recon
    mag_total_inst  = mag_left_instrumental_recon + mag_right_instrumental_recon

    # apply smoothing to mask with variable exponent
    gamma = 1.0
    vocal_mask = (mag_total_vocal ** gamma) / ((mag_total_vocal ** gamma) + (mag_total_inst ** gamma) + epsilon)
...
for i in range(n_samples):
    ...
    v_ref_cropped = v_ref[:, :min_len, :]
    v_hat = v_hat[:, :min_len, :]

    # compute BSS performance metrics
    (SDR, ISR, SIR, SAR, perm) = museval.metrics.bss_eval(v_ref_cropped, v_hat)

    print(f'Our model results for sample {i+1}:')
    ...
    # (1) using the original mix as the vocal estimate
    mix_stereo, _ = librosa.load(full_sample_paths[i], mono=False, sr=None)  # (2, T)
    mix_stereo = mix_stereo.T  # (T, 2)

    mix_b   = mix_stereo[np.newaxis, :, :]

    min_len_b = min(v_ref.shape[1], mix_b.shape[1])
    v_ref_b = v_ref[:, :min_len_b, :]
    mix_b   = mix_b[:, :min_len_b, :]

    SDR_mix, ISR_mix, SIR_mix, SAR_mix, _ = museval.metrics.bss_eval(v_ref_b, mix_b)
    print(f'Baseline mix for sample {i+1}:')
    print(f'  SDR_mix: {np.nanmean(SDR_mix)}')
    print(f'  SAR_mix: {np.nanmean(SAR_mix)}')
    
    
    # (2) using HPSS
    mix_ch_first = mix_stereo.T
    harm = np.zeros_like(mix_ch_first)

    # insert comment about HPSS
    for ch in range(mix_ch_first.shape[0]):
        y_ch = mix_ch_first[ch]  # (T,)
        y_harm, y_perc = librosa.effects.hpss(y_ch)
        harm[ch] = y_harm  # keep harmonic part per channel

    harm_stereo = harm.T

    v_hat_hpss = harm_stereo[np.newaxis, :, :]  # (1, T, 2)

    min_len_h = min(v_ref.shape[1], v_hat_hpss.shape[1])
    v_ref_h   = v_ref[:, :min_len_h, :]
    v_hat_hpss = v_hat_hpss[:, :min_len_h, :]

    SDR_hpss, ISR_hpss, SIR_hpss, SAR_hpss, _ = museval.metrics.bss_eval(v_ref_h, v_hat_hpss)
    print(f'Baseline HPSS for sample {i+1}:')
    print(f'  SDR_hpss: {np.nanmean(SDR_hpss)}')
    print(f'  SAR_hpss: {np.nanmean(SAR_hpss)}')
    '''
...
# write CSV
csv_path = 'musdb18_eval_results.csv'
